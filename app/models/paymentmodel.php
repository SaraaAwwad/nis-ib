...
class PaymentModel extends AbstractModel
{
    ...
    {
        $query = "INSERT INTO
        payment(user_id_fk, amount, method_id_fk, currency_id_fk, semester_id_fk,status_id_fk, date)
        VALUES (:user_id_fk, :amount, :method_id_fk, :currency_id_fk, :semester_id_fk,:status_id_fk, :date)";
    ...
    public static function PaymentID($student_id, $semester_id)
    {
        $query = "SELECT id FROM payment where user_id_fk = $student_id AND semester_id_fk = $semester_id";
        $stmt = self::prepareStmt($query);
        if($stmt->execute()){
